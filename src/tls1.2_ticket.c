...
void * tls12_ticket_auth_init_data() {
    ...
    randombytes_buf(global->local_client_id, 32);
    return global;
}
...
int tls12_ticket_pack_auth_data(tls12_ticket_auth_global_data *global, server_info *server, char *outdata) {
    ...
    outdata[2] = t >> 8;
    outdata[3] = t;
    randombytes_buf((uint8_t*)outdata + 4, 18);
...
#define CSTR_DECL(name, len, str) const char* name = str; const int len = sizeof(str) - 1;
        ...
        char sni[256] = {0};
        char* param = NULL;
        if (self->server.param != NULL && strlen(self->server.param) > 0)
        ...
        randombytes_buf(tls_data + tls_data_len, 208);
        tls_data_len += 208;
        ...
        pdata += 5;
        randombytes_buf((uint8_t*)pdata, 22);
        pdata += 22;
